...
- **If you change `cli/**`, do not bump CLI version files just to trigger publish**: the CLI workflow auto-bumps the patch version on `main` when the checked-in version is already published. Only change `cli/package.json` / `cli/package-lock.json` when the release metadata itself needs to change.
