...
const ENV_SUCCESS = {
  ...
  config: {
    ...
    githubClientId: "Iv1.fake",
    githubClientSecret: "secret-123",
    redisRestUrl: "https://redis.example.com",
    redisRestToken: "token-123",
    byokActiveKeyVersion: "v1",
    byokMasterKeysJson: "{\"v1\":\"00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff\"}",
    siteUrl: "https://hivemoot.dev",
    nodeEnv: "test",
  },
};
...
beforeEach(() => {
  ...
  vi.mocked(resetBranchToSha).mockResolvedValue({
    ref: "refs/heads/hivemoot-role-edits",
    sha: "base-sha-xyz",
  });
  vi.mocked(writeFileToBranch).mockResolvedValue(undefined);
  vi.mocked(createPullRequest).mockResolvedValue({
    number: 99,
    title: "chore(roles): update agent role instructions via hivemoot.dev",
    headRef: "hivemoot:hivemoot-role-edits",
    ...
    vi.mocked(listOpenPRsForBranch).mockResolvedValue([
      {
        ...
        title: "chore(roles): update agent role instructions via hivemoot.dev",
        headRef: "hivemoot:hivemoot-role-edits",
        url: "https://github.com/hivemoot/hivemoot/pull/7",
      },
    ]);
